<?php require_once("../prices/index.php"); ?>
<script>
$('#myModal').modal('show');
</script>
    ...
    <form id="signup-form" class="form-horizontal" action="../includes/signup-process.php" method="post">
      ...
      <div id="email" class="control-group">
        ...
        <div class="controls">
          ...
          rel="popover" data-content="Please enter your email address. It must be in the following format: john@doe.com" data-original-title="Email Address" data-trigger="focus" autocomplete="off">
        </div>
      </div>
      ...
      <!-- address ends -->
      <div id="SSN" class="control-group">
        <label class="control-label" for="inputSSN">Last 4 Digits of SSN*</label>
        <div class="controls">
          <input name="ssn" type="number" id="inputSSN" placeholder="" onkeyup="validateSSN();"
          rel="popover" data-content="Please enter the last 4 digits of your Social Security Number" data-original-title="SSN" data-trigger="focus">
        </div>
      </div>
      ...
      <?php require_once("TOS.php"); ?>
        ...
        <div id="ageVerify" class="control-group" style="margin-top:-20px;">
        <label class="checkbox">
        <div  class="controls">
        <span id="checkbox18Span" class="">I am 18 years or older</span> </label>
         <input id="checkboxAge" type="checkbox"> 
        </div>
         </div>
...
$(document).ready(function(){
	$("#signup-form").submit(function(){
		...
		validateAddress2();
		validateSSN();
		validateCheckBox ('#checkboxAge', '#ageVerify');
		
		if (validateFirstName() && validateLastName() && validateEmail() && validateUsername() && validatePassword() &&			validatePassword2() &&	validateCheckBox ('#checkboxTOS', '#tos') && validateDOB() && validateAddress2() && validateSSN() && validateCheckBox ('#checkboxAge', '#ageVerify')) {
			//alert ("works");
			$.ajax({
				url: $(this).attr('action'),
				type: $(this).attr('method'),
